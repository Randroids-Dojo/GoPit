...
async def test_ultimate_charge_starts_at_zero(game):
    ...
    charge = await game.get_property("/root/GameManager", "ultimate_charge")
    assert charge == 0.0
...
async def test_ultimate_charge_max_constant(game):
    ...
    max_charge = await game.get_property("/root/GameManager", "ULTIMATE_CHARGE_MAX")
...
async def test_add_ultimate_charge(game):
    ...
    initial = await game.get_property("/root/GameManager", "ultimate_charge")
    await game.call("/root/GameManager", "add_ultimate_charge", [10.0])
    after = await game.get_property("/root/GameManager", "ultimate_charge")
...
async def test_ultimate_charge_caps_at_max(game):
    ...
    # Add more than max
    await game.call("/root/GameManager", "add_ultimate_charge", [150.0])
    charge = await game.get_property("/root/GameManager", "ultimate_charge")
...
async def test_is_ultimate_ready_when_full(game):
    ...
    # Initially not ready
    ready_before = await game.call("/root/GameManager", "is_ultimate_ready")
    assert ready_before == False

    # Fill it up
    await game.call("/root/GameManager", "add_ultimate_charge", [100.0])
    ready_after = await game.call("/root/GameManager", "is_ultimate_ready")
...
async def test_use_ultimate_when_ready(game):
    ...
    # Use it
    result = await game.call("/root/GameManager", "use_ultimate")
    assert result == True
...
async def test_use_ultimate_when_not_ready(game):
    ...
    # Add partial charge
    await game.call("/root/GameManager", "add_ultimate_charge", [50.0])
...
async def test_ultimate_charge_per_kill_constant(game):
    ...
    charge_per_kill = await game.get_property("/root/GameManager", "CHARGE_PER_KILL")
...
async def test_ultimate_charge_per_gem_constant(game):
    ...
    charge_per_gem = await game.get_property("/root/GameManager", "CHARGE_PER_GEM")
