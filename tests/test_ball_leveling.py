...
async def test_get_unowned_ball_types(game):
    ...
    assert len(unowned) == 12, "Should have 12 unowned ball types initially"

    # Add BURN
    await game.call(BALL_REGISTRY, "add_ball", [1])

    # Now should have 11 unowned
    unowned_after = await game.call(BALL_REGISTRY, "get_unowned_ball_types")
    assert len(unowned_after) == 11, "Should have 11 unowned after adding BURN"
