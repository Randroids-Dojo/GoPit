...
async def test_get_unowned_ball_types(game):
    ...
    await reset_ball_registry(game)
    # Get unowned types (should be 16: all except BASIC which is auto-added on game start)
    # Ball types: BASIC, BURN, FREEZE, POISON, BLEED, LIGHTNING, IRON, RADIATION, DISEASE, FROSTBURN, WIND, GHOST, VAMPIRE, BROOD_MOTHER, DARK, CELL, CHARM
    unowned = await game.call(BALL_REGISTRY, "get_unowned_ball_types")
    assert len(unowned) == 16, "Should have 16 unowned ball types initially"

    # Add BURN
    await game.call(BALL_REGISTRY, "add_ball", [1])

    # Now should have 15 unowned
    unowned_after = await game.call(BALL_REGISTRY, "get_unowned_ball_types")
    assert len(unowned_after) == 15, "Should have 15 unowned after adding BURN"
